...
class SegLayer(tf.keras.layers.Layer):
    def __init__(self, model, out_shape, **kwargs):
        super(SegLayer, self).__init__(**kwargs)

        self.model = model
        self.out_shape = out_shape

    def call(self, inputs, training=False):
        return self.model(inputs)[:, :, :, :-1]  # 不需要background

    def compute_output_shape(self, input_shape):
        return (input_shape[0], 128, 128, self.out_shape)


class PoseLayer(tf.keras.layers.Layer):
    def __init__(self, model, out_shape, **kwargs):
        super(PoseLayer, self).__init__(**kwargs)

        self.model = model
        self.out_shape = out_shape

    def call(self, inputs, training=False):
        return self.model(inputs)  # 不需要background
...
def ensemble_model(output_bias, channels, two_residual_block, event_detector_mod, color_seg, a2c_chamber_seg, a3c_chamber_seg, a4c_chamber_seg):
    inp_a2c = Input((20, 128, 128, 3), name="a2c")
    inp_a3c = Input((20, 128, 128, 3), name="a3c")
    inp_a4c = Input((20, 128, 128, 3), name="a4c")
    inp_a2c_color = Input((20, 128, 128, 3), name="a2c_color")
    inp_a3c_color = Input((20, 128, 128, 3), name="a3c_color")
    inp_a4c_color = Input((20, 128, 128, 3), name="a4c_color")

    #####
    inp_a2c_time_embed = ReshapeLayer((-1, 20, 128*128, 3))(inp_a2c)
    inp_a2c_time_embed = add_coords_layer_frame(inp_a2c_time_embed)
    inp_a2c_time_embed = ReshapeLayer(
        (-1, 20, 128, 128, 4))(inp_a2c_time_embed)

    a2c_raw_embed_merge = TransposeLayer((0, 3, 2, 1, 4))(inp_a2c_time_embed)
    a2c_raw_embed_merge = TransposeLayer((0, 2, 1, 3, 4))(
        a2c_raw_embed_merge)  # 256,256,20,16

    a2c_raw_temp = tf.keras.layers.Conv3D(
        128, 3, strides=2, padding='same', name='conv_0_a2c_raw')(a2c_raw_embed_merge)

    a2c_raw_temp = tf.keras.layers.LeakyReLU()(a2c_raw_temp)

    a2c_raw_temp = ConvAttentionLayer()(a2c_raw_temp)

    a2c_raw_temp_1 = CspResBlock(channels, two_residual_block, name='csp_0_a2c_raw')(
        a2c_raw_temp)  # 128,128,10,128
    a2c_raw_temp_1 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a2c_raw_temp_1)  # 64,64,5,128

    a2c_raw_temp_2 = CspResBlock(channels, two_residual_block, name='csp_1_a2c_raw')(
        a2c_raw_temp_1)  # 64,64,5,128
    a2c_raw_temp_2 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a2c_raw_temp_2)  # 32,32,2,128

    a2c_raw_temp_3 = CspResBlock(channels, two_residual_block, name='csp_2_a2c_raw')(
        a2c_raw_temp_2)  # 32,32,2,128
    a2c_raw_temp_3 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a2c_raw_temp_3)  # 8,8,1,128

    a2c_raw_reshape = tf.reshape(a2c_raw_temp_3, [-1, 8, 8, 128])

    a2c_raw_reshape = CapsAttentionLayer()(a2c_raw_reshape)
    v_for_a2c_raw = CapsuleLayer_for_final_model(
        name='capsule_output_for_a2c_raw')(a2c_raw_reshape)  # 1,2,16
    ####

    a2c_color_systole_seg_out = SystoleSeg(
        color_seg, a2c_chamber_seg, event_detector_mod, 1, name="a2c_color_systole_output")(inp_a2c_color)
    a2c_color_merge = tf.keras.layers.Concatenate(
        axis=-1)([inp_a2c_color, a2c_color_systole_seg_out])  # 20,256,256,15

    inp_a2c_color_time_embed = ReshapeLayer(
        (-1, 20, 128*128, 4))(a2c_color_merge)
    inp_a2c_color_time_embed = add_coords_layer_frame(inp_a2c_color_time_embed)
    inp_a2c_color_time_embed = ReshapeLayer(
        (-1, 20, 128, 128, 5))(inp_a2c_color_time_embed)

    a2c_color_embed_merge = TransposeLayer(
        (0, 3, 2, 1, 4))(inp_a2c_color_time_embed)
    a2c_color_embed_merge = TransposeLayer((0, 2, 1, 3, 4))(
        a2c_color_embed_merge)  # 20,256,256,5

    a2c_color_temp = tf.keras.layers.Conv3D(
        128, 3, strides=2, padding='same', name='conv_0_a2c_color')(a2c_color_embed_merge)
    a2c_color_temp = tf.keras.layers.LeakyReLU()(a2c_color_temp)
    a2c_color_temp = ConvAttentionLayer()(a2c_color_temp)

    a2c_color_temp_1 = CspResBlock(channels, two_residual_block, name='csp_0_a2c_color')(
        a2c_color_temp)  # 128,128,10,128
    a2c_color_temp_1 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a2c_color_temp_1)  # 64,64,5,128

    a2c_color_temp_2 = CspResBlock(channels, two_residual_block, name='csp_1_a2c_color')(
        a2c_color_temp_1)  # 64,64,5,128
    a2c_color_temp_2 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a2c_color_temp_2)  # 32,32,2,128

    a2c_color_temp_3 = CspResBlock(channels, two_residual_block, name='csp_2_a2c_color')(
        a2c_color_temp_2)  # 32,32,2,128
    a2c_color_temp_3 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a2c_color_temp_3)  # 16,16,1,128

    a2c_color_reshape = tf.reshape(a2c_color_temp_3, [-1, 8, 8, 128])

    a2c_color_reshape = CapsAttentionLayer()(a2c_color_reshape)
    v_for_a2c_color = CapsuleLayer_for_final_model(
        name='capsule_output_for_a2c_color')(a2c_color_reshape)  # 1,2,16

    ###
    # a3c

    inp_a3c_time_embed = ReshapeLayer((-1, 20, 128*128, 3))(inp_a3c)
    inp_a3c_time_embed = add_coords_layer_frame(inp_a3c_time_embed)
    inp_a3c_time_embed = ReshapeLayer(
        (-1, 20, 128, 128, 4))(inp_a3c_time_embed)

    a3c_raw_embed_merge = TransposeLayer((0, 3, 2, 1, 4))(inp_a3c_time_embed)
    a3c_raw_embed_merge = TransposeLayer((0, 2, 1, 3, 4))(
        a3c_raw_embed_merge)  # 256,256,20,16

    a3c_raw_temp = tf.keras.layers.Conv3D(
        128, 3, strides=2, padding='same', name='conv_0_a3c_raw')(a3c_raw_embed_merge)
    a3c_raw_temp = tf.keras.layers.LeakyReLU()(a3c_raw_temp)
    a3c_raw_temp = ConvAttentionLayer()(a3c_raw_temp)
    a3c_raw_temp_1 = CspResBlock(channels, two_residual_block, name='csp_0_a3c_raw')(
        a3c_raw_temp)  # 128,128,10,128
    a3c_raw_temp_1 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a3c_raw_temp_1)  # 64,64,5,128

    a3c_raw_temp_2 = CspResBlock(channels, two_residual_block, name='csp_1_a3c_raw')(
        a3c_raw_temp_1)  # 64,64,5,128
    a3c_raw_temp_2 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a3c_raw_temp_2)  # 32,32,2,128

    a3c_raw_temp_3 = CspResBlock(channels, two_residual_block, name='csp_2_a3c_raw')(
        a3c_raw_temp_2)  # 32,32,2,128
    a3c_raw_temp_3 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a3c_raw_temp_3)  # 8,8,1,128

    a3c_raw_reshape = tf.reshape(a3c_raw_temp_3, [-1, 8, 8, 128])

    a3c_raw_reshape = CapsAttentionLayer()(a3c_raw_reshape)
    v_for_a3c_raw = CapsuleLayer_for_final_model(
        name='capsule_output_for_a3c_raw')(a3c_raw_reshape)  # 1,2,16
    ####

    a3c_color_systole_seg_out = SystoleSeg(
        color_seg, a3c_chamber_seg, event_detector_mod, 1, name="a3c_color_systole_output")(inp_a3c_color)
    a3c_color_merge = tf.keras.layers.Concatenate(
        axis=-1)([inp_a3c_color, a3c_color_systole_seg_out])  # 20,256,256,15

    inp_a3c_color_time_embed = ReshapeLayer(
        (-1, 20, 128*128, 4))(a3c_color_merge)
    inp_a3c_color_time_embed = add_coords_layer_frame(inp_a3c_color_time_embed)
    inp_a3c_color_time_embed = ReshapeLayer(
        (-1, 20, 128, 128, 5))(inp_a3c_color_time_embed)

    a3c_color_embed_merge = TransposeLayer(
        (0, 3, 2, 1, 4))(inp_a3c_color_time_embed)
    a3c_color_embed_merge = TransposeLayer((0, 2, 1, 3, 4))(
        a3c_color_embed_merge)  # 20,256,256,5

    a3c_color_temp = tf.keras.layers.Conv3D(
        128, 3, strides=2, padding='same', name='conv_0_a3c_color')(a3c_color_embed_merge)
    a3c_color_temp = tf.keras.layers.LeakyReLU()(a3c_color_temp)
    a3c_color_temp = ConvAttentionLayer()(a3c_color_temp)
    a3c_color_temp_1 = CspResBlock(channels, two_residual_block, name='csp_0_a3c_color')(
        a3c_color_temp)  # 128,128,10,128
    a3c_color_temp_1 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a3c_color_temp_1)  # 64,64,5,128

    a3c_color_temp_2 = CspResBlock(channels, two_residual_block, name='csp_1_a3c_color')(
        a3c_color_temp_1)  # 64,64,5,128
    a3c_color_temp_2 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a3c_color_temp_2)  # 32,32,2,128

    a3c_color_temp_3 = CspResBlock(channels, two_residual_block, name='csp_2_a3c_color')(
        a3c_color_temp_2)  # 32,32,2,128
    a3c_color_temp_3 = tf.keras.layers.MaxPooling3D(
        (2, 2, 2))(a3c_color_temp_3)  # 16,16,1,128

    a3c_color_reshape = tf.reshape(a3c_color_temp_3, [-1, 8, 8, 128])

    a3c_color_reshape = CapsAttentionLayer()(a3c_color_reshape)
    v_for_a3c_color = CapsuleLayer_for_final_model(
        name='capsule_output_for_a3c_color')(a3c_color_reshape)  # 1,2,16
    ...
    a4c_color_systole_seg_out = SystoleSeg(
    ...
    ###
    dense_for_a2c_raw = tf.reshape(v_for_a2c_raw, [-1, 32])
    dense_for_a2c_color = tf.reshape(v_for_a2c_color, [-1, 32])

    dense_for_a3c_raw = tf.reshape(v_for_a3c_raw, [-1, 32])
    dense_for_a3c_color = tf.reshape(v_for_a3c_color, [-1, 32])
    ...
    ensemble_vector = tf.keras.layers.Concatenate(
        axis=-1)([dense_for_a2c_raw, dense_for_a2c_color, dense_for_a3c_raw, dense_for_a3c_color, dense_for_a4c_raw, dense_for_a4c_color])  # None,64
    ...
    model = tf.keras.Model(inputs=[inp_a2c, inp_a2c_color, inp_a3c,
                           inp_a3c_color, inp_a4c, inp_a4c_color], outputs=[out_1])

    return model


def get_model_for_MR():
    event_detector_mod = cnn_lstm()
    event_detector_mod.load_weights(
        './ensemble_model/event_detector_best_model/event_detector_128.h5')

    color_seg = create_hourglass_network(
        3, 1, 128, (128, 128), (128, 128), bottleneck_mobile)  # inflow / outflow / bg
    color_seg.load_weights(
        './ensemble_model/color_regur_best_model/MR_4_128_model.h5')

    a2c_chamber_seg = create_hourglass_network(
        3, 1, 128, (128, 128), (128, 128), bottleneck_mobile)  # atrium / ventricle / bg
    a2c_chamber_seg.load_weights(
        './ensemble_model/color_chamber_seg_best_model/A2C_chamber_4_128_model.h5')

    a3c_chamber_seg = create_hourglass_network(
        3, 1, 128, (128, 128), (128, 128), bottleneck_mobile)  # atrium / ventricle / bg
    a3c_chamber_seg.load_weights(
        './ensemble_model/color_chamber_seg_best_model/A3C_chamber_4_128_model.h5')

    a4c_chamber_seg = create_hourglass_network(
        5, 1, 128, (128, 128), (128, 128), bottleneck_mobile)  # atrium / ventricle / bg
    a4c_chamber_seg.load_weights(
        './ensemble_model/color_chamber_seg_best_model/A4C_chamber_4_128_model.h5')

    ensemble_mod = ensemble_model([-1.37125648], 32, False, event_detector_mod,
    ...
    It happens because you are restoring a model that has training information 
    (such as optimizer variables) but you are only using it for prediction, 
    not training. When predicting, you don't need the saved optimizer values, 
    which is why the program is telling you they were not used.
    '''
    ensemble_mod.load_weights(
        './ensemble_model/all_ensemble_model_2/MR_sigmoid/best_model_MR_sigmoid')
...
ensemble_mod = ensemble_model(32 , False ,event_detector_mod, color_seg, a2c_chamber_seg, a3c_chamber_seg, a4c_chamber_seg)
